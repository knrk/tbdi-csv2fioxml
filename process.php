...
?>
    <div class="mdl-layout__container layout">
      <div class="upload-card mdl-card mdl-shadow--2dp">
...
if (isset($_POST['submit'])) {
  ...
  if ($fileextentsion != 'csv') {
      echo "Jsi si jistý, že opravdu nahráváš CSV soubor ?";
  } else {
      if (move_uploaded_file($_FILES["csvfilename"]["tmp_name"], $inputFilename)) {
          echo "<p>Soubor <b>" . basename($_FILES["csvfilename"]["name"]) . "</b> byl úspěšně nahrán na server.</p>";
      } else {
          echo "O ou, vyskytla se chyba se uploadu souboru.";
      }
...
if (file_exists(FIO_XML_FILE)) {
?>
            <p>FIO XML se podařilo správně vytvořit.</p>
          </div>
          <div class="mdl-card__actions mdl-card--border">
            <a href="javascript:history.go(-1);" class="mdl-button mdl-button--raised" name="submit" type="submit">Zpět</a>
            <a href="?download=true" class="mdl-button mdl-button--raised mdl-button--colored" name="submit" type="submit">Stáhnout FIO XML</a>
          </div>
<?php
} else {
 ?>
          ...
          <div class="mdl-card__actions mdl-card--border">
            <a href="javascript:history.go(-1);" class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect" name="submit" type="submit">Zpět</a>
          </div>
<?php
}
?>
